...
## calculate Cellcluster correlation
eta_l = []
for i in range(kernal_influence.shape[-1]):
    eta = correlation_ratio(anno_color.Cellcluster, kernal_influence.iloc[:,i])
    eta_l.append(eta)

eta_comb_l = []
for i in range(comb_influe.shape[-1]):
    eta = correlation_ratio(anno_color.Cellcluster, comb_influe.iloc[:,i])
    eta_comb_l.append(eta)

data=pd.DataFrame({"type":np.hstack([["eta"]*kernal_influence.shape[-1], 
                                    ["eta_comb"]*comb_influe.shape[-1]]), 
                    "eta":np.hstack([eta_l, eta_comb_l])})
data.to_csv("./Influence/eta_Cellcluster.csv")

plt.figure(figsize=(6, 10))
sns.boxplot(x="type", y="eta", data=data)
plt.savefig("./Influence/eta_boxplot_Cellcluster.pdf")
